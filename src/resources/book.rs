...
#[inline(always)]
fn extract_get(req: &Request<ServerState>) -> GetBookByTitlePayload {
    GetBookByTitlePayload {
        title: req.param("title").unwrap(),
    }
}
respond_with_actor::generate!(Config {
    name: get,
    actor: Book,
    response_type: Book,
    tag: GetBookByTitle
...
#[inline(always)]
pub async fn get_by_title(msg: &GetBookByTitleMsg) -> Result<Option<Book>, sqlx::Error> {
